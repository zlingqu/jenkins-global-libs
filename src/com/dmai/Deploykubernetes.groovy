...
class Deploykubernetes {
    ...
    public void deployKubernetes() {
        ...
        // 先创建configMap
        if (!(this.conf.appName in ['meta-adapter', 'meta-service','storage-service', 'vod-service', 'stat-service', 'sync-service','dispatcher-service', 'ui-backend-service', 'engine-audio-process', 'engine-metric-service'])) {
