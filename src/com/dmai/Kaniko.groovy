...
class Kaniko {
    ...
    public void makeAndPushImage() {
        this.script.sh String.format('#!/busybox/sh \n pwd && /busybox/tree -L 2 && /kaniko/executor --ignore-path=/busybox --ignore-path=/kaniko -v info -c ${WORKSPACE} -f ${WORKSPACE}/Dockerfile --build-arg MODEL_VERSION=%s --build-arg FRONTEND_ENV=%s --build-arg VUE_APP_SCENE=%s --destination %s',
