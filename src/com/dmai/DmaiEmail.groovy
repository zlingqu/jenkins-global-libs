...
class DmaiEmail {
    ...
    private String emailBody(String buildResult) {
        ...
        // def apkViewUrlQrcode = (this.script.sh String.format("curl -s ci-test.devops.dev.dm-ai.cn/qrcode?url=$s|base64", apkViewUrl))
        ...
        if (conf.getAttr('codeLanguage') == 'android') {
            ...
            this.script.sh String.format('''curl -s ci-test.devops.dev.dm-ai.cn/qrcode?url=$s|base64 > apkViewUrlQrcode.txt''', this.conf.appName)
