...
//            ],
		...
		'xmc-storage-service': [
            ...
            'tk-storage-image': [
                    ...
                    'test': true, // 是否从dev分支部署到测试环境
                    'master': 'prd', // 如果参数master 不等于prd，整个构建就失败，---
                    'gitAddress': 'https://gitlab.dm-ai.cn/XMC/xmc-tk/xmc-storage-image.git',
           ...
           'tk-offline-image-extract': [
                    ...
                    'test': true, // 是否从dev分支部署到测试环境
                    'master': 'prd', // 如果参数master 不等于prd，整个构建就失败，---
                    'gitAddress': 'https://gitlab.dm-ai.cn/XMC/xmc-tk/xmc-offline-image-extract.git',
                    'stage': false, //是否部署到stage环境
                    'compile': false, // 是否编译
                    'deploy': true, // 是否自动化部署
                    'customDockerfile': true, // 是否使用自定义 dockerfile
            ...
            'tk-backend-system-config': [
                    ...
                    'test': true, // 是否从dev分支部署到测试环境
                    'master': 'prd', // 如果参数master 不等于prd，整个构建就失败，---
                    'gitAddress': 'https://gitlab.dm-ai.cn/XMC/xmc-tk/xmc-backend-system-config.git',
                    'stage': false, //是否部署到stage环境
                    'compile': false, // 是否编译
                    'deploy': true, // 是否自动化部署
                    'customDockerfile': false, // 是否使用自定义 dockerfile
...
//                    'test': true, // 是否从dev分支部署到测试环境
//                    'master': 'prd', // 如果参数master 不等于prd，整个构建就失败，---
//                    'gitAddress': 'https://gitlab.dm-ai.cn/XMC/xmc-tk/xmc-engine-asr-service.git',
//                    'useModel': true,
//                    'modelPath': '', //模型文件path, 在构建的时候相对于当前的代码的主目录
//                    'compile': false, // 是否编译
//                    'deploy': true, // 是否自动化部署
...
//                    'memoryLimits' : '4000Mi',
                    ...
                    'test': true, // 是否从dev分支部署到测试环境
                    'master': 'prd', // 如果参数master 不等于prd，整个构建就失败，---
                    'gitAddress': 'https://gitlab.dm-ai.cn/research-algorithm/CV/Demo/xmc-body-action.git',
                    'compile': false, // 是否编译
                    'deploy': true, // 是否自动化部署
                    'customDockerfile': true, // 是否使用自定义 dockerfile
                    'customKubernetesDeployTemplate' : false, // 是否使用用户自定义的k8s部署文件，默认文件名为：Deploy-k8s.yml
...
//                    'nodePort' : '30332',
                    'containerPort': '3000',
                    'domain': 'dispatcher-service.dm-ai.cn', // domain为空，或者没有这条属性，则邮件不发送域名，否则给用户发送域名地址。
//                    'cpuRequests' : '1000m',
//                    'memoryRequests' : '2000Mi',
                    'cpuLimits' : '500m',
                    'memoryLimits' : '1000Mi',
