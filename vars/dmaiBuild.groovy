...
//    if (conf.getAttr('branchName') == 'master' && conf.getAttr('master') !='prd') return
    ...
    //
    def deployEnv =  Tools.addItemToListHead(['prd', 'dev', 'test', 'stage', 'jenkins', 'mlcloud-dev', 'xmc2-lexue','not-deploy'], conf.getDeployEnv())

    // default node env
    def defaultNodeEnvList = Tools.addItemToListHead(['dev', 'prod', 'test', 'stage', 'jenkins', 'mlcloud-dev', 'xmc2-lexue','not-deploy'], conf.getDeployEnv() ? conf.getDeployEnv().replaceAll('prd', 'prod') : 'dev')
