                        ...
                        steps {
                            container('adp') {
                                script {
                                    try {
                                        withCredentials([usernamePassword(credentialsId: 'devops-use', passwordVariable: 'password', usernameVariable: 'username')]) {
                                            sh "source /etc/profile; echo $password $username;git config --global http.sslVerify false ; git checkout master ;git remote -v; git fetch ;git checkout ${conf.getAttr('gitTag')}"
                                        }
                                    } catch (e) {
                                        sh 'echo ${e}'
                                        conf.failMsg = '拉取指定git的版本或者tag失败，请检查版本或者tag是否正确，请确保tag是从master分支拉取。'
